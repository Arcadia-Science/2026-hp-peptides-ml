    ...
    def __init__(
        ...
        if internal_weights and self.weight_numel > 0:
            ...
            for ins in self.instructions:
                if ins.has_weight:
                    # Support path_shape lengths != 3 by flattening to 2D.
                    if len(ins.path_shape) == 1:
                        left_dim = ins.path_shape[0]
                        right_dim = 1
                    else:
                        left_dim = prod(ins.path_shape[:-1])
                        right_dim = ins.path_shape[-1]
                    self.LoRA_weight.append(torch.nn.Parameter(torch.randn(left_dim, self.r)))
                    self.LoRA_weight.append(torch.nn.Parameter(torch.zeros(self.r, right_dim)))
            self.LoRA_weight_numel = sum(
                (prod(ins.path_shape[:-1]) if len(ins.path_shape) > 1 else ins.path_shape[0]) * self.r
                + self.r * (ins.path_shape[-1] if len(ins.path_shape) > 1 else 1)
                for ins in self.instructions
                if ins.has_weight
            )
        else:
